...
//Guitar.prototype.strumChord = function(time, downstroke, velocity, chord, arpeggioDelay) {
Guitar.prototype.strumChord = function(chord, options) {
    options = options || {};
    var time = options.time || 0;
    time = audioCtx.currentTime + time;
    var downstroke = options.downstroke || true;
    var velocity = options.velocity || 1.0;
    var duration = options.duration || 3.0;
    var arpeggioDelay = options.arpeggioDelay || 0;
    ...
    for (var i = 0; i < 6; i++) {
        ...
        if (chord[stringNumber] != -1) {
            this.strings[stringNumber].pluck(time, velocity, chord[stringNumber], duration);
            time += arpeggioDelay;
        }
...
Guitar.prototype.arpeggio = function(chord) {
    //this.strumChord(chord, {time: 0, downstroke: true, velocity: 1.0, arpeggioDelay: 0.5});
    this.strumChord(chord, {arpeggioDelay: 0.5});
}
